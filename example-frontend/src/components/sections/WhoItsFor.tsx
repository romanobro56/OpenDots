  ...
  return (
    <section ref={ref} className="bg-cream py-24 lg:py-32">
      <div className="container mx-auto px-6">
        <h2
          className={`font-display font-black text-3xl sm:text-4xl lg:text-5xl text-navy text-center mb-16 transition-all duration-700 ${
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {audiences.map((a, i) => (
            <div
              key={i}
              className={`bg-background rounded-3xl p-8 border-t-4 ${a.accent} shadow-md transition-all duration-700 ${
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
              }`}
              style={{ transitionDelay: `${i * 200}ms` }}
            >
              <a.icon className="w-10 h-10 text-gold mb-6" strokeWidth={1.5} />
              <h3 className="font-display font-extrabold text-lg text-navy mb-4">{a.title}</h3>
